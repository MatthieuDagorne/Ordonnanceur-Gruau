...
class RulesEngine:
    ...
    - centre_de_charge_id: code du centre de charge (ex: PLI01)
    ...
    def _get_applicable_rules(self, tache_id: str, centre_de_charge_id: str, article_id: str = None) -> List[BusinessRule]:
        ...
        for rule in applicable_rules:
            ...
            if rule.machine_id == machine_id:
                if rule.rule_type == RuleType.FORBID:
                    allowed = False
                    reason = f"INTERDIT par '{rule.name}'"
                    reasons.append(reason)
                    self._log_application(tache_id, centre_de_charge_id, rule, machine_id, "BLOQUE")
                    
                elif rule.rule_type == RuleType.ALLOW:
                    reason = f"AUTORISE par '{rule.name}'"
                    reasons.append(reason)
                    self._log_application(tache_id, centre_de_charge_id, rule, machine_id, "AUTORISE")
                    
                elif rule.rule_type == RuleType.PREFER:
                    preference_score += 100
                    reason = f"PREFEREE par '{rule.name}' (+100)"
                    reasons.append(reason)
                    self._log_application(tache_id, centre_de_charge_id, rule, machine_id, "PREFEREE")
    ...
    def get_allowed_machines(
        ...
        logger.info(f"      Critères: tâche={tache_id}, centre={centre_de_charge_id}, article={article_id or '-'}")
